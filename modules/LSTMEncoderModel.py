...
class LSTMEncoder(nn.Module):
    def __init__(self, vocab, config, pretrained_embedding):
        ...
        self.turn_lstm = MyLSTM(
            input_size=config.lstm_hiddens * 2 + config.role_dims,
            hidden_size=config.lstm_hiddens,
            num_layers=config.lstm_layers,
            batch_first=True,
            bidirectional=True,
    ...
    def forward(self, words, extwords, roles, sent_masks, turn_masks):
        ...
        sent_represents = self.avg_pooling(sent_lstm_hiddens, sent_masks)
        ...
        diaglog_represents = self.avg_pooling(turn_lstm_hiddens, turn_masks)  # b hidden

        return diaglog_represents

    def avg_pooling(self, data, mask):
        denominator = torch.sum(mask, -1) + 1e-20
        assert len(data.size()) == 3
        assert len(denominator.size()) == 1
        sum_data = torch.sum(data, 1)
        avg_data = torch.div(sum_data.transpose(1, 0), denominator).transpose(1, 0)
        return avg_data.contiguous()


    def att(self, data, mask):
        return
