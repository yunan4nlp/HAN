...
class CHANEncoder(nn.Module):
    def __init__(self, vocab, config, pretrained_embedding):
        ...
        self.sent_att = WordAttention(vocab.l1_size, config.lstm_hiddens * 2)
        #self.sent_att = Attention(config.lstm_hiddens * 2)
        self.turn_att = WordAttention(vocab.l1_size, config.lstm_hiddens * 2)
    ...
    def forward(self, words, extwords, roles, sent_masks, turn_masks):
        ...
        sent_lstm_hiddens = sent_lstm_hiddens.unsqueeze(0).repeat(self.l1_size, 1, 1, 1)
        sent_represents = self.sent_att(sent_lstm_hiddens, sent_masks)

        sent_represents = sent_represents.view(self.l1_size, b, turn_size, -1).contiguous()
        ...
        turn_masks = turn_masks.unsqueeze(0).repeat(self.l1_size, 1, 1).view(-1, turn_size)
